...
func check_selected_resolution() -> void:
	var width : int = ProjectSettings.get("display/window/size/width") as int
	var height : int = ProjectSettings.get("display/window/size/height") as int
...
func switched(id : int) -> void:
	...
	if label != "Add custom...":
		...
		ProjectSettings.set("display/window/size/width", data.size.x)
		ProjectSettings.set("display/window/size/height", data.size.y)
		if menu.text == "Custom-Portrait (630x1120)":
			print("Addon: resolutionChanger | init.gd, switched(id:%s) menu.text == 'Custom-Portrait'"%[id])
			ProjectSettings.set("display/window/size/test_width", 315)
			ProjectSettings.set("display/window/size/test_height", 560)
